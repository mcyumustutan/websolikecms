<div class="col-lg-3">
    <div class="card rounded">
        <img class="card-img-top" src="{{asset('images/omer-eren.jpg')}}" alt="Card image cap">

        <div class="card-header rounded text-center">
            <h5 class="card-title">ÖMER EREN</h5>
            <p class="card-subtitle">T.C. Göreme Belediye Başkanı</p>

        </div>
        <div class="card-body  ">
            <p class="mb-2">
                <a class="card-link" href="{{config('app.url')}}/tr/kurumsal/baskan">
                    Özgeçmiş <i class="ri-arrow-right-up-line"></i>
                </a>
                <a class="card-link" href="{{config('app.url')}}/tr/kurumsal/mesaj">
                    Başkandan Mesaj <i class="ri-arrow-right-up-line"></i>
                </a>
            </p>
            <p class="card-text">
                <a target="_blank" href="https://www.instagram.com/baskanomereren/">
                    <i class="ri-instagram-line fs-4"></i>
                </a>
                <a target="_blank" href="{{$settings['facebookcom']}}">
                    <i class="ri-facebook-line fs-4"></i>
                </a>
                <a target="_blank" href="https://x.com/baskanomereren">
                    <i class="ri-twitter-line fs-4"></i>
                </a>
            </p>
        </div>
    </div>
</div>